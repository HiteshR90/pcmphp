	...
	if(!isset($_SESSION['session_user'])||$_SESSION['session_user']=="") {
		...
		if($rows>0) {
			try {
				...
				$result = $conn->prepare("UPDATE pcm_show SET city=:city,detail=:detail,show_date=CONVERT_TZ(:date, :timeZone, '+00:00'),venue=:venue,time_zone=:timeZoneId,cost=:cost,link_to_buy=:link,contact_venue=:contact,address=:address where id=:showId");
